...
TOOL_DECLARATIONS = [
    ...
    {"name": "live_vision", "description": "Gemini Live vision control. For instant visual questions like 'what am I holding?', 'look at this', 'what do you see on camera?', 'what's on my screen?', call action='snapshot' (mode='camera' or mode='desktop') ONCE to immediately capture and inject an exact-moment frame into the session, then directly describe what you see. For continuous stream: action='enable' (mode='camera'|'desktop'|'both'), action='disable', action='status'.",
     "parameters": {"type": "OBJECT", "properties": {
         "action": {"type": "STRING", "description": "snapshot (instant one-shot frame, default for visual questions), enable, disable, status, enable_camera, enable_desktop, enable_both"},
         "mode": {"type": "STRING", "description": "camera (default), desktop, or both — used with snapshot/enable"},
    ...
    {"name": "screen_agent", "behavior": "NON_BLOCKING",
     "description": (
         ...
         "on the screen by their appearance, CLICKS them, and READS/summarises the result. "
         "Use whenever the user wants to interact with or read something visible on screen: "
         "'check notifications on PW', 'click the notification bell', 'read what is on screen', "
         ...
         "For read-only screen description, use action=read_screen or action=screenshot_and_describe."
     ),
     "parameters": {"type": "OBJECT", "properties": {
         "action": {
             "type": "STRING",
    ...
    {"name": "file_controller", "behavior": "NON_BLOCKING", "description": "Unified file, folder, and autonomous organization workflows: organize (sorts loose files in Downloads, Desktop, or any folder into categorized subfolders Documents/Images/Code/Archives/etc.), create_folder, open_folder, delete, move (relocate a file or folder on disk), copy, rename, batch_rename (pattern-based sequential or find-replace renaming), compress (zip files/folders), extract (unzip archives), clean_temp (safe temp cache cleanup), clean_empty (remove empty directories), list, find. 'move' here means relocating files on disk — NOT moving the mouse cursor (use mouse_actions) and NOT rearranging windows (use advanced_automation window_arrange). delete is destructive and requires verbal_confirmed=true.",
     "parameters": {"type": "OBJECT", "properties": {"action": {"type": "STRING", "description": "organize, create_folder, open_folder, delete, move, copy, rename, batch_rename, compress, extract, clean_temp, clean_empty, list, find"}, "path": {"type": "STRING", "description": "Target path or folder e.g. 'downloads', 'desktop', or full path"}, "src": {"type": "STRING"}, "dest": {"type": "STRING"}, "root": {"type": "STRING"}, "pattern": {"type": "STRING", "description": "Glob pattern for find or batch_rename, e.g. '*.pdf'"}, "prefix": {"type": "STRING", "description": "Prefix for batch_rename"}, "find": {"type": "STRING", "description": "Search string for batch_rename find-and-replace"}, "replace": {"type": "STRING", "description": "Replacement string for batch_rename find-and-replace"}, "verbal_confirmed": {"type": "BOOLEAN", "description": "Required for delete. Set true only after the user has clearly said yes out loud to a spoken confirmation."}}, "required": ["action"]}},
    ...
    {"name": "automation_engine", "behavior": "NON_BLOCKING", "description": "Goal-driven multi-step batch automation for natural-language workflows — e.g. 'organize my desktop', 'compress all images in Downloads', 'extract every zip archive', 'bulk-rename screenshots'. Delegates to file_controller for file operations and system tools for OS routines. Destructive results (lock/shutdown/restart/sleep/hibernate) require confirmation_code AND verbal_confirmed.",
     "parameters": {"type": "OBJECT", "properties": {
    ...
    {"name": "telegram_sender", "behavior": "NON_BLOCKING", "description": "Send Telegram messages, Live native-audio voice notes, or files via Bot API. Actions: send (DEFAULT for all text messages like 'send on telegram', 'send my schedule on telegram', 'message on telegram' — uses message= OR regarding=class_schedule/reminders/notes with day=today|tomorrow|week, default day=today), send_voice/voice (ONLY when user explicitly asks for a voice note / voice message / audio / 'bol ke bhejo' — uses message= OR regarding=), schedule_voice (pre-synthesize Live audio now, send exactly at at= time or in_minutes= — for 'send voice message regarding X at Y'), list_scheduled, cancel_scheduled (id=), send_file (path=), setup, status, test, enable_alerts, disable_alerts. For send_voice: takes 10-20s — say you are sending now, NEVER say sent/delivered until a later tool result confirms success.",
     "parameters": {"type": "OBJECT", "properties": {
         "action": {"type": "STRING", "description": "send (text message, default), send_voice (voice note only if requested), schedule_voice, list_scheduled, cancel_scheduled, send_file, setup, status, test, enable_alerts, disable_alerts"},
         "message": {"type": "STRING", "description": "Text message content to send or voice script to speak"},
         "text": {"type": "STRING", "description": "Alias for message"},
         "regarding": {"type": "STRING", "description": "class_schedule | reminders | notes | clipboard — dynamic context lookup (default day=today for schedule)"},
         "day": {"type": "STRING", "description": "today | tomorrow | week | next | monday… (defaults to today)"},
         "use_schedule": {"type": "BOOLEAN", "description": "If true, load real class_schedule data (default day=today)"},
    ...
    {"name": "goal_tracker", "behavior": "NON_BLOCKING", "description": "Long-horizon goal tracking (JARVIS-style) — goals span days/weeks, have subtasks (owner: gama|user|either), progress, deadlines, and proactive check-ins. Unlike reminder (single-turn) or task_queue (one batch). Actions: create (title, description?, deadline?, priority? 1-10, subtasks? list), get/status (id — full detail + subtasks), list (status: active|paused|done|abandoned|all), update (id, note?, progress_pct?), add_subtask (id, description, owner?), update_subtask / complete_subtask (subtask_id, status?, result?), break_down (id — auto-create starter subtasks), advance (id? — next ready GAMA-owned subtask for autonomous work), checkin (id), complete/pause/resume/abandon/delete (id), history (id).",
     "parameters": {"type": "OBJECT", "properties": {
         "action": {"type": "STRING", "description": "create|get|list|update|add_subtask|update_subtask|complete_subtask|break_down|advance|checkin|complete|pause|resume|abandon|delete|history"},
         "id": {"type": "INTEGER", "description": "Goal id (or subtask_id for subtask actions)"},
         ...
         "status": {"type": "STRING"},
         "owner": {"type": "STRING", "description": "gama|user|either for subtasks"},
         "priority": {"type": "INTEGER"},
         "result": {"type": "STRING"},
         "subtasks": {"type": "STRING", "description": "Optional semicolon-separated initial subtasks on create"}
     }, "required": ["action"]}},
    ...
    {"name": "self_diagnostics", "behavior": "NON_BLOCKING", "description": "Read Gama's error logs in real-time and diagnose issues in natural human language. Use whenever the user asks 'what error happened?', 'why did it fail?', 'check your logs', 'what's the error?'. Actions: explain_last_error (plain-English explanation & diagnosis of the latest error/crash), recent_errors (list recent caught errors/warnings with timestamps), read_logs (read recent log lines from gama.log), crash_summary (persistent crash reports), status (health/loop status).",
     "parameters": {"type": "OBJECT", "properties": {
         "action": {"type": "STRING", "description": "explain_last_error, recent_errors, read_logs, crash_summary, status"},
         "limit": {"type": "INTEGER", "description": "Number of errors or crash entries to return (default 5)"},
         "lines": {"type": "INTEGER", "description": "Number of lines for read_logs (default 30)"},
         "level": {"type": "STRING", "description": "Minimum log level: ERROR, WARNING, INFO (default ERROR)"}
     }, "required": ["action"]}},
    {"name": "self_repair", "behavior": "NON_BLOCKING", "description": "Safe Code Self-Inspection, Editing, & On-Demand Repair. Allows Gama to read its own source files, apply validated patches/fixes, check Python syntax (AST parsing), and revert changes if needed. Edits are strictly sandboxed to Gama's codebase, automatically backed up to storage/backups/code/, and validated before writing. Actions: read_code (path, start_line, end_line), edit_code (path, target_content, replacement_content), revert (path, backup?), verify_syntax (path), list_backups (path?).",
     "parameters": {"type": "OBJECT", "properties": {
         "action": {"type": "STRING", "description": "read_code, edit_code, revert, verify_syntax, list_backups"},
         "path": {"type": "STRING", "description": "Codebase file path (e.g. 'actions/telegram_sender.py' or 'core/tool_dispatch.py')"},
         "start_line": {"type": "INTEGER", "description": "Start line number for read_code (default 1)"},
         "end_line": {"type": "INTEGER", "description": "End line number for read_code"},
         "target_content": {"type": "STRING", "description": "Exact text chunk in the file to replace (must match exactly)"},
         "replacement_content": {"type": "STRING", "description": "New replacement code to write"},
         "allow_multiple": {"type": "BOOLEAN", "description": "True to replace all occurrences instead of only 1 (default false)"},
         "backup": {"type": "STRING", "description": "Optional specific backup filename to restore for revert"}
     }, "required": ["action"]}}
]
...
ALWAYS_TOOLS: frozenset[str] = frozenset({
    "open_app", "reminder", "display_stage", "d2_mode",
    "recall_memory", "save_memory", "remember", "forget_memory",
    "desktop_context",
    "user_settings", "clipboard", "system_info",
    "shutdown_assistant",
    "set_voice", "goal_tracker",
